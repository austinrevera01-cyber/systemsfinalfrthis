...
function controller = controller_dev_local(params, velocity, SS_values, plot_opts)
    ...
    zeta       = 1.05;     % extra damping to suppress limit-cycle tendency
    TTS_delta  = 0.22;     % steering loop target settle time [s]
    TTS_r      = 0.32;     % yaw-rate loop target settle time [s]
    ...
    controller.Ki3 = omega_n_psi^2 * 0.35;  % slower integrator to avoid sustained oscillations
    ...
    % Ask pidtune for a higher phase margin to keep the cascaded loop well
    % damped once the heading integral action starts pushing harder.
    tune_opts = pidtuneOptions('PhaseMargin', 75);
    C_r_pd    = pidtune(C_r_plant, 'PD', omega_n_r, tune_opts);
